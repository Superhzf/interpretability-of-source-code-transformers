...
def extract(args, model, tokenizer,debug=True):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_dataset = TextDataset(tokenizer, args,args.extract_data_file)
...
def main():
    ...
    parser.add_argument("--extract_data_file", default=None, type=str,
                        help="An optional input extraction data file to extract the activations on (a text file).")
    ...
    parser.add_argument('--sentence_only', action='store_true', help="For Extracting sentence-level embeddings( only [CLS] token embeddings in case of BERT-like models)")
